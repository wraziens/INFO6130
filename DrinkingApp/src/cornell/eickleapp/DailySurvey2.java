...
public class DailySurvey2 extends Activity implements OnClickListener {
	...
	/*
	 * Clears the current values in selected_values and Checks to see which of
	 * the checkboxes are checked. Adds the corresponding string values to
	 * selected_values for the options that are checked.
	 */
	...
	public void onClick(View view) {
		...
		case R.id.wordle_finish:
			...
			if (selected_values.size() > 0) {
				...
				saveToDb();
				db.updateOrAdd("DailySurvey2" + "CheckList", "done");
				finish();
			} else {
				Toast.makeText(getApplicationContext(),
						"Please select at least one", Toast.LENGTH_SHORT)
						.show();
			}
			break;
		}
