...
public class Visualize extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// get chicken counts
		ArrayList<DatabaseStore> chicken_count_vals = (ArrayList<DatabaseStore>) db
				.getVarValuesForDay("number_chickens", date);
		Integer chicken_count = 0;
		if (chicken_count_vals != null && chicken_count_vals.size() == 1) {
			DatabaseStore c_count = chicken_count_vals.get(0);
			chicken_count = Integer.parseInt(c_count.value);
		}
	...
	public void onClick(View arg0) {
		...
		/*
		 * switch(arg0.getId()){ case R.id.ibDataUpdate: Time timeNow=new
		 * Time(); timeNow.setToNow(); String time3339=timeNow.format3339(true);
		 ...
		 * query.orderByAscending("createdAt"); query.findInBackground(new
		 * FindCallback<ParseObject>() {
		 * 
		 * @Override public void done(List<ParseObject> drinkList,
		 * ParseException e) { if (e == null) { String drinkResult=""; int
		 * totalSec=0; int initialSec=0; for (int n=0;n<drinkList.size();n++){
		 * int tempSec=0;
		 * tempSec=tempSec+Integer.parseInt(drinkList.get(n).getString
		 * ("hour"))*360;
		 * tempSec=tempSec+Integer.parseInt(drinkList.get(n).getString
		 * ("minute"))*60;
		 * tempSec=tempSec+Integer.parseInt(drinkList.get(n).getString
		 * ("second")); if (n==0){ initialSec=tempSec; totalSec=tempSec; } else
